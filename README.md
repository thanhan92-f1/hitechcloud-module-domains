...
- WHOIS: `whoisDomain()`
- `whoisDomain()` normalize best-effort các field phổ biến như `domain`, `registrar`, `created_at`, `updated_at`, `expires_at`, `statuses`, `nameservers`, `contacts`
- hỗ trợ parse WHOIS text response thành cấu trúc dễ dùng hơn nếu API không trả JSON chuẩn
...
- DNS records:
  ...
  - `updateDNSManagement()` nhận tốt hơn các field thay thế như `host`, `subdomain`, `record_type`, `target`, `value`, `address`, `ttl`
- DNSSEC:
  - `widget_dnssec_form()`
  - `widget_dnssec_get()`
...
- `GET /whois/:domain`
- WHOIS được chuẩn hóa best-effort từ cả JSON lẫn raw text response
